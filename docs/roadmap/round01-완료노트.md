...
- 운영 도구 5개(`build_dev_images.sh`·`build_gongu_manifest.py`·`build_wikisource_manifest.py`·`crawler.py`·`build_vlm_policy_selection.py`)는 `scripts/`에 잔류(+ `scripts/bulk_ingest/`)
- 디버그 로그 4개 삭제 + `.gitignore` 재발 방지 패턴 추가(`*_debug_out*.txt`·`soffice_log*.txt`·`debug_*.txt`)
- README §5(프로젝트 구조)를 `docs/`·`scripts/`·`research/` 신설 반영해 갱신 — 실존하지 않는 `migrate_add_KCI.sql` 참조를 실제 4개 파일(`migrate_add_book_figures.sql`·`migrate_add_cover_image.sql`·`migrate_add_introduction.sql`·`migrate_add_themes.sql`)로 정정
...
- `research/`로 옮긴 스크립트 다수(약 26개)가 입출력 경로를 여전히 옛 `scripts/...` 위치로 하드코딩(19개는 입력부터 못 찾아 실행 자체가 실패, 7개만 산출물이 엉뚱한 곳에 다시 생기는 증상 — Task 10·11 리뷰 발견). 코드 수정은 이번 라운드(파일 재배치) 스코프 밖 — 다음에 그 스크립트들을 실제로 재실행할 일이 생기면 그때 입출력 상수를 `Path(__file__).parent` 기준으로 고친다(다른 주제 폴더를 참조하는 20건은 자기 폴더 기준만으로는 부족).
- `scripts/build_vlm_policy_selection.py`가 이제 `research/`로 이동된 파일들을 옛 경로로 참조해 실행하면 실패할 수 있음 — 이 스크립트 자체가 1회성 연구 스크립트였을 가능성의 근거(Task 12 참고).

## 다음 라운드 진입점
- 범위 미정 — `docs/roadmap/00_status.md`와 `README.md` §10 로드맵에서 확인

## 상태
- [x] code-reviewer 정적 리뷰 통과(발견 사항 반영 완료 — Task 1~13 전 태스크 spec-compliance + code-quality 2단계 서브에이전트 리뷰, 발견 시 즉시 수정)
