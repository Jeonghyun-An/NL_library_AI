...
- **증상**: 참고문헌 복구 스크립트가 대상 41,399건을 다 돌았는데도, 화면에 참고문헌이 비어 보이는 논문이 남는다. 검증 쿼리는 37,463건이 "참고문헌 있음"이라고 보고한다.
- **원인**: 적재 파이프라인(`app/services/ingestion/stages.py`)이 초록·목차·키워드 중 하나만 있어도 `extra["references"]` 를 **빈 리스트째로** 기록한다. 대상 선정 조건 `NOT (extra ? 'references')` 는 키 존재만 보므로 `[]` 로 저장된 문서가 영구히 제외됐고, 같은 조건을 쓴 검증 쿼리는 보유율을 부풀렸다.
- **해결**: 존재가 아니라 길이로 판정한다 — `jsonb_array_length(coalesce(extra->'references','[]'::jsonb)) = 0`.
- **재발 방지**: jsonb 배열·객체 필드에 `?` 를 쓸 땐 "키가 있는가"와 "값이 비었는가"가 다른 질문임을 먼저 확인한다. 쓰는 쪽이 빈 값을 기록하는지 코드로 확인한다.
