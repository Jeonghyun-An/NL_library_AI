"""audit_routing_lit.py — 문학작품 표본(KDC 800~899)에 대한 라우팅 규칙 감사.

학술논문 100건(2,024쪽)에서 얻은 재배정률 0.30%가 자료 유형에 따라 어떻게 달라지는지
확인하기 위해, 동일한 절차를 문학작품 100건에 적용한다. 판정 공식·정제 로직은
audit_routing_fix.py 와 완전히 동일하며, fitz 추출량은 별도 패스(scripts/fitz_lengths_lit.py)에서 수집해 병합한다.

문서 단위로 중간저장하므로 중단 후 다시 실행하면 남은 문서만 처리한다.

사용법:
    python scripts/audit_routing_lit.py
"""
import io
import json
import os
...
BASE = "C:/Users/LANDSOFT/Desktop/SKOVIX/output1/pdfs/"
SAMPLE = "C:/Users/LANDSOFT/mygit/NL_library_AI/scripts/lit_sample100.txt"
OUT = "C:/Users/LANDSOFT/mygit/NL_library_AI/scripts/audit_routing_lit100.json"
