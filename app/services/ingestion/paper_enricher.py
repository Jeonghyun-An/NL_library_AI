...
_REF_HEADER = re.compile(
    ...
    # 한 줄에 붙어 나오는 경우. '#' 마크다운 헤더 마커가 실제로 있고
    # 키워드가 줄 끝에 오는 경우만 허용(막연한 본문 오매칭 방지).
    r'|^.*#{1,6}\s*(?:' + _REF_HEADER_KEYWORD + r')\s*[*_#]*\s*$'
    r')',
    re.IGNORECASE,
)
...
_REF_ENTRY = re.compile(
    ...
    r'|[가-힣]{2,4}\((?:19|20)\d{2}\)'    # 단독 한국어 저자명(연도) — 구분자 없음 (박성수(1997).)
    r'|[A-Z][a-zA-Z\-]{2,}\((?:19|20)\d{2}\)'  # 단독 영문 성(연도) — 쉼표 없음 (Csikszentmihalyi(1965).)
    r')'
)
