...
- `docs/` = 문서 전용.
- `scripts/` = **재실행되는 운영 도구**만 둔다 — 파이프라인이 지금도 호출하거나 반복 실행하는 것(`bulk_ingest/`, 매니페스트 빌더 등).
...
## 2. 개발 어휘 — round
- **round**: 사용자 프롬프트 1개 → 구현 → dev → 리뷰 → dev→main 머지 + push까지가 한 라운드. 라운드당 spec 1(브레인스토밍) + 완료노트 1 + 교본(가이드) 1권.
...
| 기능 설계(브레인스토밍 spec) | `docs/superpowers/specs/` |
