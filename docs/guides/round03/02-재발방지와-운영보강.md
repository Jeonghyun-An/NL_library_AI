...
이 라운드에서 새로 만든 테스트 파일 2개와, 기존 파일에 추가·재작성한 테스트다. 전체는 123 passed(로컬 미설치 패키지 `FlagEmbedding`·`openpyxl` 로 collect 실패하는 3개 모듈 제외).
...
#### `app/tests/test_paper_enricher.py` (기존 파일 — `TestExtractReferences` 클래스에 5개 추가)
