...
> **부록 — 설계 도중 발견한 선행 결함 (round01 착수 전 별도 조치 완료)**: 이 spec을 커밋하려는 과정에서 `.gitignore`의 `/docs`가 저장소 생성 이후 `docs/` 전체를 한 번도 추적하지 못하게 막고 있었음을 발견했다(`.claude/`도 동일, `/scripts`는 2026-09-03 커밋에 무관하게 추가돼 그 이후 신규 스크립트가 미추적). round01 워크플로우 설계와 무관한 선행 결함이라 즉시 별도로 `.gitignore`를 고치고 그동안 쌓인 `docs/`·`.claude/settings.json`·`scripts/`의 미추적 파일을 커밋 `3743453`으로 백업했다(§2의 "0단계 캐치업"은 이 조치가 끝난 상태를 전제로 한다).
