...
- 산출물 위치 규칙: `research/<주제>/`로 118개(scripts) + 6개(root) 파일 이동, 이후 리뷰로 4건 재분류
...
- Note: this round went through 2-stage subagent review (spec-compliance + code-quality) for every task, catching and fixing multiple real issues (documented in each Task's plan header annotation) — reference that this is why some files show a fix-commit chain rather than a single commit. 실제로 Task 1(`f5983bb`→`11815ae`, Q6 참조)과 research 재분류(`c61f427`·`fb309e7`, §2.9 참조)가 그 예다.
...
A. 파일을 옮기는 작업 자체는 기계적이지만, "이 파일이 어느 주제에 속하는가"는 내용을 읽어야 판단할 수 있다. 최초 설계 spec은 파일명 패턴으로 주제를 배정했는데, 실제 코드품질 리뷰가 각 파일의 docstring·출력 경로·의존 관계를 읽어보니 4~7건이 잘못 배정돼 있었다(예: `space_stats_full.py`는 이름만 보면 OCR 비교 같지만 내용은 표 셀 통계라 table-cell-fill 소속). 매 이동 Task마다 리뷰를 거쳤기 때문에 이런 오분류가 라운드 완료 전에 잡혔다.
