"""fitz_lengths_lit.py — 문학작품 표본 100건의 fitz 단독 추출량.

ODL은 프로젝트 .venv, fitz는 별도 환경에만 설치되어 있어 한 프로세스에서 함께 쓸 수 없다.
그래서 라우팅 감사(audit_routing_lit.py)와 분리해 실행하고, 분석 단계에서
(파일, 페이지) 기준으로 병합한다. 정제 로직은 감사 스크립트와 동일하다.

사용법:
    python scripts/fitz_lengths_lit.py
"""
import io
import json
import os
...
OUT = "C:/Users/LANDSOFT/mygit/NL_library_AI/scripts/fitz_lengths_lit100.json"


def _clean_text(text: str) -> str:
    """audit_routing_lit.py 와 동일."""
    text = re.sub(r'\n{3,}', '\n\n', text)
    lines = []
